...
\]
and the Hamiltonian $\mathcal{H}: l^2(\Z,\R^2) \rightarrow \R$ is
...
\end{equation}
where $u(t) \in l^2(\Z, \R^{2k})$, $\mathcal{H}: l^2(\Z, \R^{2k}) \rightarrow \R$ is smooth with $\mathcal{H}(0) = 0$ and $\mathcal{H}'(0) = 0$, and $J$ is a $2k \times 2k$ symplectic matrix. For simplicity, and again using dNLS as motivation, we will assume that $\mathcal{H}'(u)$ takes the form
...
\mathcal{H}'(u) = 0.
\end{equation}
Differentiating the symmetry invariance \eqref{Hinvariance}, we obtain the symmetry relation
...
We are interested in bound states, which are solutions to \eqref{lattice1} of the form $u(t) = R(\omega t)u$, where $u \in l^2(\Z, \R^{2k})$ is independent of $t$. Bound states satisfy the equilibrium equation
...
For $\omega \in (\omega_1, \omega_2)$, there exists a $C^1$ map $\omega \mapsto q$ such that $q \in l^2(\Z, \R^{2k})$ is a bound state solution to \eqref{latticestat}.
\end{hypothesis}
...
Since $F(0) = 0$, 0 is an equilibrium point for the dynamical system \eqref{diffeq}. Fix $\omega \in \omega_1, \omega_2)$, and let $q$ be the bound state from Hypothesis \ref{boundstatehyp} corresponding to $\omega$. Let $Q(n) = ( q_n, q_{n-1} )$. Since $q \in l^2(\Z, \R^{2k})$, $q_n \rightarrow 0$ as $n \rightarrow \pm \infty$, thus $Q(n)$ is a homoclinic orbit solution to $\eqref{diffeq}$ connecting the equilibrium at 0 to itself. We will refer to this as the primary pulse solution. It follows from \eqref{Lkernel2} that
