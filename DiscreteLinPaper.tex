...
In this paper, we will write dNLS as a system of two real variables $(v_n, w_n)$, where for convenience we retain the notation $u_n = (v_n, w_n)$. In this fashion, we can write \eqref{dNLS} in Hamiltonian form as
\begin{equation}\label{dNLSrealHam}
\frac{d}{dt}\begin{pmatrix}u_n\end{pmatrix}
+ J \mathcal{H}'\begin{pmatrix}u_n\end{pmatrix} = 0,
\end{equation}
where $J$ is the standard skew-symmetric symplectic matrix
\[
J = \begin{pmatrix}0 & 1 \\ -1 & 0\end{pmatrix}
\]
and the Hamiltonian $\mathcal{H}: l^2 \rightarrow \R$ is
\begin{equation}\label{dNLSrealH}
\mathcal{H}(v_n, w_n) = -\sum_{n = -\infty}^\infty 
...
\mathcal{Q}(v_n, w_n) = \frac{1}{2} \sum_{n = -\infty}^\infty 
\left( v_n^2 + w_n ^2\right).
\end{equation}

Standing waves of \eqref{dNLSrealHam} are of the form $R(\omega t) u_n$, which we can substitute into \eqref{dNLSrealHam} to obtain the equivalant system of equations
\begin{equation}\label{dNLSreal1}
\mathcal{H}'(u_n) - \omega u_n = 0,
\end{equation}
which is given by
...
\end{aligned}
\end{equation}
If $u_n$ is a standing wave solution, then $R(\theta) u_n$ is also a standing wave by symmetry. The steady state system has the form
\begin{equation}
\mathcal{H}(v_n, w_n) - \omega \mathcal{Q}(v_n, w_n) = 0,
\end{equation}
...
For stability analysis, the linearization of \eqref{dNLSrealHam}about a standing wave solution $u_n^*$ is the eigenvalue problem $(L(u_n^*) - \lambda I) p_n = 0$, where $L(u_n^*)$ is the linear operator 
\begin{equation}\label{dNLSeigproblem}
L = J \mathcal{H}''(u_n^*)  - \omega J
\end{equation}
Let $u_n^* = (q_n, 0)$, where $q_n$ is the real-valued, on-site standing wave solution to \eqref{dNLS}. For the generalized kernel of $L$, it is straightforward to verify that
\begin{equation}\label{dNLSkernel1}
\begin{aligned}
L (R'(0) u_n^*) &= 0 \\
L (\partial_\omega u_n^* ) &= R'(0) u_n^*
\end{aligned}
\end{equation}

For $d \neq 0$, if we let $\tilde{v}_n = v_{n-1}$ and $\tilde{w}_n = w_{n-1}$, we can write \eqref{dNLSequilibreal} as the lattice dynamical system
\begin{equation}\label{DNLSlattice1}
U_{n+1} = F(U_n; \omega),
\end{equation}
where $U_n = (v_n, w_n, \tilde{v}_n, \tilde{w}_n)$ and $F:\R^4 \times \R \rightarrow \R^4$ is smooth. Similary, we can write the eigenvalue problem as
\begin{equation}\label{dNLSlatticeEVP}
V_{n+1} = DF(Q_n; \omega) V_n + \lambda B V_n
\end{equation}
where $B$ is the constant coefficient block matrix 
...
In the next section, we will study a generalization of the lattice dynamical systems \eqref{DNLSlattice1} and \eqref{dNLSlatticeEVP}. These arise naturally from a Hamiltonian lattice differential equation as the existence problem for bound states and the linearization of the lattice differential equation about a bound state. We will return to dNLS in Section \ref{sec:dNLS}.
...
\partial_t u(n, t) = J \mathcal{H}'(u(n, t))
\end{equation}
where $u(n, \cdot) \in l^2(\Z, \R^{2d})$, $\mathcal{H}: l^2(\Z, \R^{2d}) \rightarrow \R$ is smooth with $\mathcal{H}(0) = 0$ and $\mathcal{H}'(0) = 0$, and $J$ is a $2d \times 2d$ symplectic matrix. For simplicity, we will assume that $\mathcal{H}'(u(n))$ takes the form
\begin{equation}\label{latticeform}
\mathcal{H}'(u(n)) = d \Delta_2 u(n) + f(u(n))
\end{equation}
where $\Delta_2$ is the second difference operator $\Delta_2 u(n) = u(n+1) - 2 u(n) + u(n-1)$ and $f: \R^{2d} \rightarrow \R^{2d}$ is a smooth function with $f(0) = 0$. We note that $Df(U(n))$ is self-adjoint.

We make the following hypothesis concerning symmetries of the system.
\begin{hypothesis}\label{symmetryhyp}
There is unitary group of symmetries $\{ R(\theta) : \theta \in \R\}$ on $\R^{2d}$ such that 
\begin{enumerate}[(i)]
\item The Hamiltonian $\mathcal{H}$ is invariant under $R(\theta)$, i.e. 
\begin{equation}\label{Hinvariance}
\mathcal{H}(R(\theta)u_n) = \mathcal{H}(u_n)
\end{equation}
\item $R'(0) = J$, where $R'(0)$ is the infinitesimal generator of $T(\theta)$.
\end{enumerate}
\end{hypothesis}
For dNLS, $R(\theta)$ is the rotation group \eqref{Rtheta}.

Equilibrium solutions to \eqref{lattice1} satisfy 
\begin{equation}\label{equilib1}
\mathcal{H}'(u(n)) = 0.
\end{equation}
Differentiating the symmetry invariance \eqref{Hinvariance}, we have the symmetry relation
\begin{equation}\label{symmetryrel}
\mathcal{H}'(R(\theta)u(n))R(\theta) = \mathcal{H}'(u(n)),
\end{equation}
from which it follows that $u(n)$ is a solution to \eqref{equilib1} if and only if $R(\theta)u(n)$ is a solution. We also note that $f(R(\theta)u) = F(u)R(\theta)$.

We are interested in bound states, which are solutions to \eqref{lattice1} of the form $u(n, t) = R(\omega t)u(n)$, where $u_n \in l^2(\Z, \R^{2d})$. Bound states satisfy the equilibrium equation
\begin{equation}\label{latticestat}
\mathcal{H}'(u(n)) - \omega u(n) = 0,
\end{equation}
and we note that if $q(n)$ is a bound state, $R(\theta)q(n)$ is also a bound state. Let $q(n)$ be a bound state solution to \eqref{latticestat}. The linearization of \eqref{lattice1} about a bound state $q(n)$ is the eigenvalue problem $L(q(n))v(n) = \lambda v(n)$, where $L(q(n))$ is the linear operator
\begin{equation}\label{latticeL}
L(q(n)) = J \mathcal{H}''(q(n)) - \omega J
\end{equation}
By substituting $R(\theta)q(n)$ into \eqref{latticestat} and differentiating with respect to $\theta$ at $\theta = 0$, we can verify that 
\begin{equation}\label{Lkernel1}
L(q(n)) R'(0) q(n) = 0
\end{equation}

As in \cite{Grillakis1987}, we take the following hypothesis about the existence of bound state solutions.
\begin{hypothesis}\label{boundstatehyp}
For $\omega \in (\omega_1, \omega_2)$, there exists a $C^1$ map $\omega \mapsto q(n; \omega)$ such that $q(n; \omega) \in l^2(\Z, \R^{2d})$ is a bound state solution to \eqref{latticestat}.
\end{hypothesis}

By Hypothesis \ref{boundstatehyp}, for $\omega \in (\omega_1, \omega_2)$, $\partial_\omega q(n)$ exists and satisfies 
\begin{equation}\label{Lkernel2}
L(q(n))\partial_\omega q(n) = R'(0) q(n).
\end{equation}
We note that this requires $R'(0) = J$.

\subsection{Spatial dynamics formulation}

Using a spatial dynamics technique, we will write the bound state equation \eqref{latticestat} as the first order difference equation
\begin{equation}\label{diffeq}
U(n+1) = F(U(n)),
\end{equation}
where $U(n) = (u(n), \tilde{u}(n)) = (u(n), u(n-1)) \in \R^{4d}$ and
...
Similarly, we can write the eigenvalue problem $(L(q(n)) - \lambda I)v(n) = 0$ as the first order difference equation 
\begin{equation}\label{latticeEVP}
V(n+1) = DF(q(n)) V(n) + \lambda B V(n),
\end{equation}
where
\begin{equation}\label{latticeF}
DF(q(n)) = \begin{pmatrix}u \\ \tilde{u} \end{pmatrix} =
\begin{pmatrix}
2 + \frac{\omega}{d} - \frac{1}{d}Df(q(n)) & -I  \\
I & 0
\end{pmatrix} 
\end{equation}
...
\end{pmatrix}.
\end{equation}
It follows from \eqref{Lkernel1} that for $Q(n) = (q(n), q(n-1))$,
\begin{equation}\label{DFkernel1}
T'(0) Q(n+1) = DF(q(n)) T'(0) Q(n+1).
\end{equation}

We note that $F(0) = 0$, thus 0 is an equilibrium point for the dynamical system \eqref{diffeq}. Fix $\omega \in \omega_1, \omega_2)$, and let $q(n)$ be the bound state from Hypothesis \ref{boundstatehyp} corresponding to $\omega$. Let 
\begin{equation*}
Q(n) = ( q(n), q_{n-1}).
\end{equation*}
Since $q(n) \in l^2(\Z, \R^{2d})$, $q(n) \rightarrow 0$ as $n \rightarrow \pm \infty$, thus $Q(n)$ is a homoclinic orbit solution to $\eqref{diffeq}$ connecting the equilibrium at 0 to itself. We will refer to this as the primary pulse solution. It follows from \eqref{Lkernel2} that
\begin{equation}\label{DFkernel2}
\partial_\omega Q(n+1) = DF(q(n)) \partial_\omega Q(n) + B T'(0)Q(n)
\end{equation}

We make the following hypothesis regarding $F$.
\begin{hypothesis}\label{initialhyp}
We assume the following regarding $F$.
\begin{enumerate}[(i)]
...
\item The tangent spaces of the stable and unstable manifolds $W^s(0)$ and $W^u(0)$ have a one-dimensional intersection at $Q(n)$ spanned by $T'(0) Q(n)$.
\end{enumerate}
\end{hypothesis}

By the stable manifold theorem, since $Q(n)$ connects the stable and unstable manifolds, we have the decay rate
\begin{equation}\label{Qdecay}
|Q(n)| \leq C r^{-|n|}
\end{equation}

By Hypothesis \ref{initialhyp}(iii), $T'(0) Q(n)$ is the unique bounded solution to the variational equation
\begin{align*}
V(n+1) &= D_U F(q(n)) V(n),
\end{align*}
where
...
\end{equation}
It follows that there exists unique bounded solution $Z_1(n)$ to the adjoint variational equation
\begin{align*}
Z(n) &= D_U F(q(n))^* Z(n+1).
\end{align*}
...
We are interested in multi-pulses, which are bound states which resemble multiple, well separated copies of the primary pulse.
First, we will give criteria for the existence of multi-pulses. We will characterize a multi-pulse solution in the following way. Let $m > 1$ be the number of copies of $Q(n)$; $N_i$ ($i = 1, \dots, m-1$) be the distances (in lattice points) between consecutive copies; and $\theta_i = G$ ($i = 1, \dots, m$) be symmetry parameters associated with each copy of $Q(n)$. We seek a solution which can be written piecewise in the form 
...
Assume Hypotheses \ref{symmetryhyp} and \ref{initialhyp}, and let $Q(n)$ be the primary pulse solution to \eqref{diffeq}. Then, for sufficiently large $N$, there exists a unique $m-$pulse solution $Q_m(n)$ to \eqref{diffeq} if and only if the $m$ jump conditions 
...
We remark that if equation \eqref{diffeq} has a conserved quantity, i.e. a function $E: \R^{4d} \rightarrow \R$ such that $E(F(U)) = E(U)$, we can remove one of the jump conditions in \eqref{jumpcondexist} as in \cite{SandstedeStrut}.

\subsection{Eigenvalue Problem}

We will now look at the spectral stability of multi-pulse solutions to \eqref{diffeq}. Let $Q_m(n) = (q_m(n), q_m(n-1))$ be an $m-$pulse solution to \eqref{diffeq} constructed according to Theorem \ref{ntmulti}. By Theorem \ref{ntmulti}, $Q_m(n)$ can be written piecewise in the form \eqref{Upiecewise}, and the estimates \eqref{Westimates} on $W_i^\pm$ from Theorem \ref{transversemulti} hold. For the eigenvalue problem, we take $q(n) = q_m(n)$ in equation \eqref{latticeEVP} to get
...
Assume Hypotheses \ref{symmetryhyp}, \ref{initialhyp}, and \eqref{melnikovhyp}. Let $Q_m(n) = (q_m(n), q_m(n-1))$ be an $m-$pulse solution to \eqref{diffeq} constructed according to Theorem \ref{ntmulti}. Then there exists $\delta > 0$ small with the following property. There exists a bounded, nonzero solution $V(n)$ of the eigenvalue problem \eqref{multiEVP} for $|\lambda| < \delta$ if and only if $E(\lambda) = 0$, where
...
\subsection{Tranverse intersection}

We will state and prove one more result, which concerns the existence of multi-pulse solutions in the case where the stable manifold $W^s(0)$ and unstable manifold $W^(u)$ intersect transversely, as opposed to the one-dimensional intersection in Hypothesis \ref{initialhyp}(iii). This is particularly useful for dNLS, as this is the case when we consider real-valued solutions to \eqref{dNLS}. For $u_n$ real, \eqref{dNLS} is an equation in $l^2(\Z, \R)$ and cannot be Hamiltonian. 

Consider the difference equation
\begin{equation}\label{diffeqtransv}
U(n+1) = F(u(n))
\end{equation}
where $F: \R^d \rightarrow \R^d$ is smooth. We make the following hypotheses concerning $F$.

\begin{hypothesis}\label{transversehyp}
We make the following hypotheses concerning $F$.
\begin{enumerate}
\item There exists a finite group $G$ (which may be the trivial group) for which the group action is a unitary group of symmetries $T(\theta)$ on $\R^d$ such that 
\begin{equation}\label{symmetrytransverse}
F(T(\theta)U) = F(U)T(\theta)
\end{equation}
for all $\theta \in G$ and all $U \in \R^d$. 
\item 0 is a hyperbolic equilibrium for $F$, thus there exists a radius $r > 1$ such that for all eigenvalues $\nu$ of $DF(0)$, $|\nu| \leq 1/r$ or $|\nu| > r$.
\item $\dim E^s, \dim E^u \geq 1$, where $E^s$ and $E^u$ are the stable and unstable eigenspaces of $DF(0)$.
...
\end{enumerate}
\end{hypothesis}
We note that for dNLS, the group $G$ is $( \{\pm 1\}, \cdot)$.
. In this case, Lin's method yields a unique $m-$pulse solution to \eqref{diffeqtransv}.

\begin{theorem}\label{transversemulti}
Assume Hypothesis \ref{symmetryhyp}(i), \ref{initialhyp}, and \ref{intersectionhyp}, and let $Q(n)$ be the primary pulse solution to \eqref{diffeq}. Then, for sufficiently large $N$, there exists a unique $m-$pulse solution $Q_m(n)$ to \eqref{diffeq} which can be written in the form \eqref{Upiecewise}. For the remainder terms $\tilde{Q}_i^\pm(n)$ we have the following estimates
\begin{equation}\label{Westimatestransverse}
\begin{aligned}
||\tilde{Q}_i^\pm|| &\leq C r^{-N} \\
\tilde{Q}_i^+(N_i^+) &= T(\theta_{i+1}) Q(-N_i^-) + \mathcal{O}(r^{-2N}) \\
\tilde{Q}_{i+1}^-(-N_i^-) &= T(\theta_i) Q(N_i^+) + \mathcal{O}(r^{-2N})
