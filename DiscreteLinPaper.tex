...
\begin{abstract}
    In the present work, we consider the existence and spectral stability of multi-pulse solutions in Hamiltonian lattice systems.
    ...
    the continuum realm. We develop explicit conditions for the existence of $m$-pulse structures and
    subsequently develop a reduced matrix allowing us to
    ...
    stability eigenvalues are obtained which are in very good agreement
    with numerical results.
\end{abstract}

\begin{keyword}
multi-pulse solutions \sep Hamiltonian lattice systems \sep lattice differential equations \sep Lin's method \MSC{39A30, 37K60}
\end{keyword}
...
R'(0) J = -J R'(0)^*,
\end{equation}
where $R'(0)$ is the infinitesimal generator of the symmetry group $R(\theta)$. In addition, since $R(\theta)$ is unitary, $R(\theta) J = J R(\theta)$. For DNLS, $R(\theta)$ is the rotation group \cref{Rtheta}. Following \cite[(2.9)]{Grillakis1987}, since $J$ is an invertible matrix, the corresponding conserved quantity is given by
\begin{equation}\label{genQ}
\mathcal{Q}(u) = \frac{1}{2} \sum_{n = -\infty}^\infty 
\langle J^{-1}T'(0) u_n, u_n \rangle
\end{equation}
...
For $\omega \in (\omega_1, \omega_2)$, there exists a $C^1$ map $\omega \mapsto q$ such that $q \in \ell^2(\Z, \R^{2k})$ is a bound state solution to \cref{latticestat}. \revised{ Furthermore, for all $\omega \in (\omega_1, \omega_2)$,
\begin{align}\label{genstabcrit}
\frac{d}{d\omega} \mathcal{Q}(q) < 0
\end{align}
}
\end{hypothesis}

\begin{remark}
The condition \eqref{genstabcrit} is the Vakhitov–Kolokolov stability criterion [CITE], which is generalized to abstract Hamiltonian systems in \cite{Grillakis1987}. Equation \eqref{genstabcrit} is equivalent to $d''(\omega) > 0$ in \cite[Theorem 3]{Grillakis1987}. In general, the condition \eqref{genstabcrit} can only be verified numerically.
\end{remark}
...
The sums involving $\tilde{H}$ give us the Melnikov sum $M_2$.
\begin{align*}
...
&= \sum_{j = -\infty}^{\infty} \langle \begin{pmatrix} J q(j) \\ -J q(j+1) \end{pmatrix}, \frac{1}{d} \begin{pmatrix} J \partial_\omega q(j) \\ 0 \end{pmatrix} \rangle \\
&= \frac{1}{d} \sum_{j = -\infty}^{\infty} \langle J q(j), J \partial_\omega q(j) \rangle \\
&= -\frac{1}{d} \sum_{j = -\infty}^{\infty} \langle q(j), \partial_\omega q(j) \rangle 
\end{align*}
...
&+ \langle T(\theta_i) Z_1(-N_{i-1}^-), T(\theta_{i-1}) T'(0)Q(N_{i-1}^+) \rangle (d_i - d_{i-1}) \\
&+\frac{1}{d} \sum_{j = -\infty}^{\infty} \langle q(j), \partial_\omega q(j) \rangle + R(\lambda)_i(d).
\end{aligned}
\end{equation}
...
\[
M_2 = \sum_{n=-\infty}^\infty \langle Z_1(n+1), B  \partial_\omega Q(n) \rangle =
\frac{1}{d} \sum_{n=-\infty}^\infty q(n) \partial_\omega q(n) = \frac{1}{d}M,
\]
where
\[
...
\bibliography{DiscreteLinPaper.bib}
